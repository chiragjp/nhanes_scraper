...
getVariableArray <- function(con, varname, series) {
	varInfo <- dbGetQuery(con, sprintf('select * from nh_var_tab where series=\'%s\' and var = \'%s\'', series, varname))
	tabName <- varInfo$tab_name
